  ...
  const handleSave = async () => {
    ...
    try {
      ...
      if (result.success) {
        ...
        setError(result.error || 'Error al guardar lección');
      }
    } catch (err: any) {
      setError(err.message || 'Error al guardar lección');
    } finally {
      setLoading(false);
    }
  };
    ...
    <TeacherLayout userName={user?.nombre}>
      <div className="max-w-5xl mx-auto space-y-6">
        ...
        <div>
          ...
          <div>
            <h1 className="text-3xl font-bold tracking-tight">Crear Nueva Lección</h1>
            <p className="text-muted-foreground mt-2">
              Completa la información básica, genera la estructura con IA y personalízala
            </p>
          </div>
        </div>
          ...
          <TabsContent value="structure" className="space-y-6">
            ...
            <Card className="border-dashed">
              <CardHeader>
                ...
                <CardDescription>
                  El agente Planner generará 6 momentos pedagógicos basados en el objetivo
                </CardDescription>
              </CardHeader>
              <CardContent>
                <Button
            ...
            {momentos.length === 0 ? (
              ...
              <div className="space-y-4">
                <div className="flex items-center justify-between">
                  <h3 className="text-lg font-semibold">Estructura Generada</h3>
                  <p className="text-sm text-muted-foreground">
                    Click para expandir y editar cada momento
                  </p>
                </div>
        ...
        <div className="flex justify-end gap-3 pb-8">
          ...
          <Button
            onClick={handleSave}
            disabled={loading || !titulo.trim() || !objetivo.trim() || momentos.length === 0}
            className="gap-2"
          >
            {loading ? (
              <>
